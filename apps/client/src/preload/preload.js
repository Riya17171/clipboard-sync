...
let jsQRModule = null;
let loadingQRCode = null;
let loadingJsQR = null;

async function getQRCode() {
  if (QRCodeModule) return QRCodeModule;
  if (!loadingQRCode) {
    loadingQRCode = import("qrcode").then((mod) => {
...
async function getJsQR() {
  ...
  if (!loadingJsQR) {
    loadingJsQR = import("jsqr").then((mod) => {
...
contextBridge.exposeInMainWorld("clipboardApp", {
  ...
  qr: {
    createDataUrl: async (text) => {
      const QRCode = await getQRCode();
      return QRCode.toDataURL(text, { margin: 2, width: 220 });
    },
    decode: (data, width, height) => {
      ...
      return jsQRModule(data, width, height);
    }
  },
  pairing: {
